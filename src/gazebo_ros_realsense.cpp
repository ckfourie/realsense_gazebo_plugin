...
// Register the plugin
    ...
    void GazeboRosRealsense::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
        ...
        if (this->depthCam && publishDepthImage_) rosnode_->setParam(rosnode_->resolveName(cameraParamsMap_[DEPTH_CAMERA_NAME].topic_name) + "/compressed/format", "png");
        if (this->depthCam && publishDepthImage_) rosnode_->setParam(rosnode_->resolveName(cameraParamsMap_[DEPTH_CAMERA_NAME].topic_name) + "/compressed/png_level", 1);

        if (this->colorCam) this->color_pub_ = this->itnode_->advertiseCamera(
                cameraParamsMap_[COLOR_CAMERA_NAME].topic_name, 2);
        if (this->ired1Cam) this->ir1_pub_ = this->itnode_->advertiseCamera(
                cameraParamsMap_[IRED1_CAMERA_NAME].topic_name, 2);
        if (this->ired2Cam) this->ir2_pub_ = this->itnode_->advertiseCamera(
                cameraParamsMap_[IRED2_CAMERA_NAME].topic_name, 2);
        if (this->depthCam && publishDepthImage_) this->depth_pub_ = this->itnode_->advertiseCamera(
                cameraParamsMap_[DEPTH_CAMERA_NAME].topic_name, 2);
        if (pointCloud_ && this->depthCam) {
    ...
    void GazeboRosRealsense::OnNewDepthFrame() {
        ...
        if (this->depth_pub_ && publishDepthImage_) {
            // copy from simulation image to ROS msg
        ...
        if (this->depthCam && pointCloud_ && this->pointcloud_pub_.getNumSubscribers() > 0) {
